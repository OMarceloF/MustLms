// src/pages/gestor/VisualizarProfessorPage.tsx
...
const handleUpdateDocument = async (documentoId: number, file: File) => {
    ...
    formData.append('documento', file); // O nome do campo deve ser 'documento'

    const url = `/api/funcionarios/${id}/documentos/${documentoId}/atualizar`;
    
    // CORREÇÃO: Adicionado options com headers para garantir multipart/form-data
    toast.promise(
      ...
      }), 
      {
        ...
        success: (response) => {
            fetchFuncionarioData(); // Recarrega os dados para mostrar o novo arquivo na lista
            return response.data.message || 'Documento atualizado com sucesso!';
        },
        error: (err: any) => {
            const message = err.response?.data?.message || 'Falha ao atualizar o documento.';
...
                        <p><strong>Gênero:</strong> {funcionario.genero || "Não informado"}</p>
...
                                <div>
                                  <p className="font-medium">{doc.tipo_documento.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</p>
