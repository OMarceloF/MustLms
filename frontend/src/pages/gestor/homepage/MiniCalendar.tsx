...
const eventStyles: any = {
  ...
  evento: { bg: 'bg-blue-100', text: 'text-blue-600', dot: 'bg-blue-500' },
  'aula-especial': { bg: 'bg-purple-100', text: 'text-purple-600', dot: 'bg-purple-500' },
...
export function MiniCalendar({ events = [], holidays = [] }: MiniCalendarProps) {
  ...
  // Verificação simples para o dia "Hoje"
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3 px-1">
          <h4 className="text-sm font-bold text-gray-800 capitalize">
            {monthNames[currentMonth.getMonth()]} <span className="text-gray-400 font-normal">{currentMonth.getFullYear()}</span>
          </h4>
          <div className="flex gap-1">
            <Button variant="ghost" size="icon" onClick={previousMonth} className="h-7 w-7 hover:bg-gray-100 rounded-full">
        ...
        <div className="grid grid-cols-7 gap-1">
          ...
          {Array.from({ length: daysInMonth }).map((_, index) => {
            ...
            const today = isToday(day); // Verifica se é hoje
            ...
            // Estilo específico para HOJE se não estiver selecionado
            // Adicionamos um anel colorido ou texto destacado
            const todayStyle = today && !isSelected ? 'ring-1 ring-[#363776] ring-offset-1 font-bold text-[#363776]' : '';
            ...
            proximosEventos.map(evento => {
               ...
               return (
                ...
                >
                  ...
                  `}>
                    {isHoliday ? <Sparkles className="h-4 w-4" /> : (
                      <>
                        <span className="text-[10px] font-bold">
                          {evento.dateObj.getDate()}
                        </span>
                  ...
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-semibold leading-tight truncate ${isHoliday ? 'text-emerald-900' : 'text-gray-800'}`}>
                      {evento.title}
                    </p>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      {isHoliday && <span className="text-[9px] text-emerald-600 font-medium">{evento.dateObj.toLocaleDateString('pt-BR')}</span>}
                    </div>
                  </div>
                </div>
              );
