...
export function VinculadosTab() {
  // O componente original usa 'cursoId', mas o PainelAcademico usa 'id' (que parece ser disciplinaId).
  // Vou manter 'cursoId' aqui, mas o PainelAcademico passará o 'id' que ele tem.
  // Se o PainelAcademico estiver em um contexto de Disciplina, o ID será o da disciplina.
  // Vou assumir que o ID passado é o contexto correto para buscar os vinculados.
  const { id: contextoId } = useParams<{ id: string }>();
  ...
  const fetchVinculados = useCallback(async () => {
    ...
    setIsLoading(true);
    try {
      // Ajustando as rotas de API para usar o contextoId
      const [alunosResponse, outrosResponse] = await Promise.all([
        axios.get(`/api/contexto/${contextoId}/alunos-vinculados`),
        axios.get(`/api/contexto/${contextoId}/vinculados`)
      ]);
      ...
      toast.error("Não foi possível carregar os dados vinculados.");
    } finally {
      setIsLoading(false);
    }
  }, [contextoId]);
  ...
  return (
    <Card className="border-border bg-card">
      <CardHeader>
        <CardTitle>Vinculados</CardTitle>
        <CardDescription>Visualize alunos, professores e turmas vinculadas ao contexto atual</CardDescription>
      </CardHeader>
      <CardContent>
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow><TableCell colSpan={3} className="text-center h-24">Nenhum aluno vinculado.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow><TableCell colSpan={3} className="text-center h-24">Nenhum professor vinculado.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </div>
              ...
              <Table>
                ...
                <TableBody>
                  {isLoading ? (
                    ...
                    <TableRow><TableCell colSpan={4} className="text-center h-24">Nenhuma turma vinculada.</TableCell></TableRow>
                  )}
                </TableBody>
              </Table>
            </div>
