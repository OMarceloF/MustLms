...
const ConfiguracoesResponsavelPage: React.FC = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (userId) {
          axios
            .get(`${API_URL}api/responsaveis/${userId}`)
            .then((resp) => {
              setNome(resp.data.nome || "");
              setLogin(resp.data.login || "");
              setEmail(resp.data.email || "");
              setNumero1(resp.data.numero1 || "");
              setNumero2(resp.data.numero2 || "");
              setEndereco(resp.data.endereco || "");
              setCpf(resp.data.cpf || "");
              setGrauParentesco(resp.data.grau_parentesco || "");
            })
            .catch(() => {
              setMensagem("Não foi possível carregar os dados do responsável.");
            });
        } else {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    if (alterarSenha) {
      ...
      try {
        const res = await axios.post(
          `${API_URL}api/responsaveis/verify-password`,
    ...
    try {
      await axios.put(
        `${API_URL}api/responsaveis/${userId}`,
        {
          nome,
          login,
          email,
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center py-10 px-2">
      ...
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl p-8 border border-indigo-300 mt-12 transition-all duration-300">
        ...
        <form onSubmit={handleSalvar} className="space-y-6">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 border rounded-lg border-indigo-400 focus:ring-2 focus:ring-indigo-600 focus:border-indigo-700 outline-none bg-indigo-50"
              />
            </div>
            <div>
            ...
            <div>
              <label className="block text-indigo-900 font-semibold mb-1">Telefone 1</label>
              <input
                type="text"
                value={numero1}
            ...
            <div>
              <label className="block text-indigo-900 font-semibold mb-1">Telefone 2</label>
              <input
                type="text"
                value={numero2}
            ...
            <div>
              <label className="block text-indigo-900 font-semibold mb-1">Endereço</label>
              <input
                type="text"
                value={endereco}
            ...
            <div>
              <label className="block text-indigo-900 font-semibold mb-1">CPF</label>
              <input
                type="text"
                value={cpf}
            ...
            <div>
              <label className="block text-indigo-900 font-semibold mb-1">Grau de Parentesco</label>
              <input
                type="text"
          ...
          {!alterarSenha && (
            <div className="flex justify-end">
              <button
                ...
                className="text-indigo-800 underline text-sm hover:text-indigo-800 transition font-medium"
              >
                Alterar senha
              </button>
            </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-6">
            ...
            <button
              type="submit"
              className="w-full sm:w-auto bg-indigo-800 text-white p-2 rounded-lg hover:bg-indigo-900 transition font-semibold shadow-md"
            >
              Salvar Alterações
            </button>
          </div>
...
export default ConfiguracoesResponsavelPage;