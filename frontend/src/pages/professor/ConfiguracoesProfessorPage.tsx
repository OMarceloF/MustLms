...
const ConfiguracoesProfessorPage: React.FC = () => {
  ...
  useEffect(() => {
    axios
      ...
      .then((res) => {
        ...
        if (userId) {
          axios
            .get(`${API_URL}api/professores/${userId}`)
            .then((profRes) => {
              setNome(profRes.data.nome || "");
              setLogin(profRes.data.login || "");
              setEmail(profRes.data.email || "");
              setDepartamento(profRes.data.departamento || "");
              setRegistro(profRes.data.registro || "");
            })
            .catch(() => {
              setMensagem("Não foi possível carregar os dados do professor.");
            });
        } else {
  ...
  const handleSalvar = async (e: React.FormEvent) => {
    ...
    try {
      await axios.put(
        `${API_URL}api/professores/${userId}`,
        {
          nome,
          email,
          login,
